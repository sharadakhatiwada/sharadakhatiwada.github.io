...
quiz.scoreStudent = function (sid) {
  let studentObj = quiz.students.find((student) => {
    return student.sid === sid;
  });
  studentObj.answers.sort(answerComparator);
  let score = getSumScoreOFAStudent(studentObj);
  return score;
};
/**
 *
 ...
 * @returns
 */
function getSumScoreOFAStudent(studentObj) {
  let count = 0;
...
quiz.getAverage = function () {
  ...
  for (let student of quiz.students) {
    let scoreOFStudent = getSumScoreOFAStudent(student);
    totalSum = totalSum + scoreOFStudent;
  }
  let average = totalSum / quiz.students.length;
  return average;
};
